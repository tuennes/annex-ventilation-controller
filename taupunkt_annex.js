...
// Compact dew-point ventilation controller for an annex.
//
// Features:
// - Read BLE sensors for indoor and outdoor measurements
// - Winter mode: ventilate only when the outdoor dew point is sufficiently higher than the indoor dew point
// - Summer mode: ventilate during a fixed daily time window regardless of humidity
// - Simple on/off switching logic
// - Remote mode switching through the Shelly app
...
var summer_start_hour = 13, summer_start_minute = 0;
var summer_end_hour = 16, summer_end_minute = 0;
var dewpoint_on_buffer_c = 1.0, dewpoint_off_buffer_c = 0.5;
...
print("Script startup: taupunkt_annex.js V1.2");
print("BLE note: The Shelly app's 'No paired devices' message is not the same as the script's BLE scan path.");
...
function isWithinSummerWindow(now) {
  var start = summer_start_hour * 60 + summer_start_minute;
  var end = summer_end_hour * 60 + summer_end_minute;
  ...
  Shelly.call("PLUGS_UI.SetConfig", {
    ...
    config: {
      leds: {
        ...
        colors: {
          "switch:0": {
            on: { rgb: [r, g, b], brightness: h },
            off: { rgb: [r, g, b], brightness: h }
          }
        }
      }
    }
...
function syncModeFromShellyApp() {
  if (!app_sync_enabled) return;
  var status = Shelly.getComponentStatus("switch:0");
  if (!status || typeof status.output === "undefined") return;
  ...
  if (status.output !== last_seen_relay_state) {
    if (suppress_next_relay_change) {
      ...
      print("Mode changed via Shelly App/Cloud to:", betrieb_modus);
    }
    last_seen_relay_state = status.output;
  }
}
...
function updateDeviceNameForApp() {
  var modeLabel = (betrieb_modus === "summer") ? "Summer" : (betrieb_modus === "winter" ? "Winter" : "Off");
  var label = "Annex " + modeLabel + " I:" + ((typeof temperatur_innen === "undefined") ? "--" : temperatur_innen.toFixed(1)) + "°C/" + ((typeof humidity_innen === "undefined") ? "--" : humidity_innen.toFixed(0)) + "% A:" + ((typeof temperatur_aussen === "undefined") ? "--" : temperatur_aussen.toFixed(1)) + "°C/" + ((typeof humidity_aussen === "undefined") ? "--" : humidity_aussen.toFixed(0)) + "%";
...
function evaluateControl() {
  var now = new Date();
  var within_summer_window = isWithinSummerWindow(now);

  if (betrieb_modus === "summer") {
    if (within_summer_window) {
      print("Summer mode: time window active - fan running.");
      setFanState(true);
      farbring(80, 10, 0, 100);
    } else {
      print("Summer mode: time window inactive - fan off.");
      setFanState(false);
      farbring(0, 0, 80, 100);
    }
    return;
  ...
  if (betrieb_modus === "off") {
    print("Mode: off - fan disabled.");
    setFanState(false);
    farbring(0, 0, 80, 100);
    return;
  }

  if (typeof taupunkt_innen === "undefined" || typeof taupunkt_aussen === "undefined" || typeof temperatur_innen === "undefined" || typeof humidity_innen === "undefined" || typeof humidity_aussen === "undefined") {
    print("Not all sensor values are available - keeping current state.");
  ...
  var should_run = (luefterstatus === true);
  var reason = "Winter mode: state unchanged";
  if (taupunkt_aussen >= taupunkt_innen + dewpoint_on_buffer_c) {
    should_run = true;
    reason = "Winter mode: outdoor dew point higher than indoor + 1 °C buffer";
  } else if (taupunkt_aussen <= taupunkt_innen + dewpoint_off_buffer_c) {
    should_run = false;
    reason = "Winter mode: dew point difference below hysteresis";
  }

  if (should_run) {
    print("Switching fan on - " + reason);
    setFanState(true);
    farbring(80, 10, 0, 100);
  } else {
    print("Switching fan off - " + reason);
    setFanState(false);
    farbring(0, 0, 80, 100);
  }
}
...
function checkBlu(event) {
  ...
  if (normalizedAddress === sensorAussenNorm) {
    ...
    print("New outdoor values:", temperatur_aussen, "°C,", humidity_aussen, "%, dew point:", taupunkt_aussen, "°C, battery:", battery_aussen, "%");
    ...
    print("New indoor values:", temperatur_innen, "°C,", humidity_innen, "%, dew point:", taupunkt_innen, "°C, battery:", battery_innen, "%");
  } else if (ble_debug) {
...
Timer.set(control_interval_seconds * 1000, true, function () {
  print("----- Control cycle every", control_interval_seconds, "s -----");
  print("Mode:", betrieb_modus, "time window:", summer_start_hour, ":", summer_start_minute, "-", summer_end_hour, ":", summer_end_minute);
...
function decodeBTHome(buffer) {
  ...
  while (bytes.length > 0) {
    ...
    if (typeof spec === "undefined") {
      print("BTH: Unknown type", bytes[0]);
      break;
    }
    bytes = bytes.slice(1);
...
function BLEScanCallback(event, result) {
  ...
  if (container && typeof container === "object") {
    for (var key in container) {
      if (typeof key === "string" && key.toLowerCase() === BTHOME_SVC_ID_STR) {
        serviceDataKey = key;
        break;
      }
    }
  ...
  if (!unpackedData || unpackedData.encryption) {
    if (ble_debug) print("BLE callback: failed to decode BTHome payload");
    return;
  }
...
function initBLE() {
  ...
  try {
    ...
    if (BLEConfig && typeof BLEConfig.enable !== "undefined") {
      if (!BLEConfig.enable) {
        print("BLE init: Bluetooth is disabled on this Shelly.");
  ...
  try {
    ...
    print("BLE init: subscription registered");
  } catch (e) { print("BLE init: subscribe failed", e); }
}

initBLE();
Timer.set(15000, true, function () {
  try {
    if (ble_supported_by_device === false) {
      print("BLE watchdog: BLE scanning is unavailable on this device/firmware.");
      return;
    }
    if (!BLE.Scanner.isRunning()) {
      print("BLE watchdog: scanner is not running, restarting...");
      initBLE();
    }
